...
<script src="{{ asset('assets/user/js/holiday.js') }}"></script>
...
@yield('vuescripts')
<script>
  let $delete_title = "{{ $keywords['Are_you_sure?'] ?? __('Are you sure?') }}";
  let $delete_subtitle =
    "{{ $keywords['You_won_not_be_able_to_revert_this!'] ?? __('You won not be able to revert this!') }}";
  let $delete_btntext = "{{ $keywords['Yes_delete_it!'] ?? __('Yes delete it!') }}";
  let $cancel_btntext = "{{ $keywords['Cancel'] ?? __('Cancel') }}";
</script>
<script>
...
@if (session()->has('success'))
  <script>
    ...
    content.title = "{{ $keywords['success'] ?? 'Success' }}";
...
@if (session()->has('warning'))
  <script>
    ...
    content.title = "{{ $keywords['warning'] ?? 'Warning' }}";
    content.icon = 'fa fa-bell';
